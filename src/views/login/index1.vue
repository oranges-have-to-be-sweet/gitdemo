      ...
      <div class="loginLeft">
        ...
        <div class="form-content">
          ...
          <div class="login-footer">
            <el-button type="primary" class="loginBtn" size="medium" @click="loginIn"
              >登录</el-button
            >
            <!-- <el-button type="text" size="small">忘记密码</el-button> -->
          </div>
          <div class="user_xy">
              <el-checkbox v-model="xy_checked"></el-checkbox>
              <div class="xy_modle">请阅读后同意<a href="https://www.wwwwwedu.com/xy/index.html" class="xy_span">《用户使用协议》</a></div>
          </div>
        </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    loginIn() {
      let _this = this;
      if(!this.xy_checked) return this.$message({type:'warning',message: "请同意用户使用协议"});
      let load = _this.$loading({
        target: document.querySelector("#login-content")
      });
      _this.$refs.loginForm.validate(valid => {
        if (valid) {
          let options = Object.assign({}, _this.loginData);
          console.log(options)
          options.password = md5(options.password) ;
          api.login.loginInApi(options)
            .then(res => {
              if (res.status == 200) {
            ...
            .then(() => {
              api.login.getUserInfo().then(res => {
                if (res.status == 200) {
                  ...
                  sessionStorage.setItem("companyId", res.data.companyList[0].id);
                  _this.$router.push("/");
                  _this.$message({
                    message: "登录成功",
                    type: "success"
